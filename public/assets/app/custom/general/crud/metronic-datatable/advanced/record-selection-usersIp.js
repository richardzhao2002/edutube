...
var KTDatatableMealTypes = function () {
    ...
    var options = {
        ...
        columns: [
            ...
            //     template: function (row) {
            //         return '\
            //         \<a href="' + location.protocol + "//" + window.location.host + '/admin/edit/' + row._id + '" class="btn btn-sm btn-clean btn-icon btn-icon-sm" title="Edit">\
    ...
    var mealTypesSelector = function () {
        ...
        $(document).on('click', '.KTStatusUpdate', function () {
            ...
            swal.fire({
                ...
                if (result.value) {
                    window.location.href = `${window.location.protocol}//${window.location.host}/admin/status-change/${elemID}/${userId}`;
                }
            });
        })
